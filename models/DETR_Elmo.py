## Note: Elmo gives 1024 dim embedding 
...
class HOIModel(nn.Module):
    """
    detr_base Backbone + Elmo text features (obj + action) + Single stage Hungarian matching.
    """
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False,
        ...
        # self.obj_class_embed = nn.Linear(hidden_dim, 1024) ## FFN head for object class pred (changed from EoID to adopt to Elmo)
        # self.verb_class_embed = nn.Linear(hidden_dim, num_verb_classes) 
        self.verb_class_embed = nn.Linear(hidden_dim, 1024) ## FFN head for verb class pred (changed from EoID to adopt to Elmo)
        ...
        self.Elmo_action_semantics = torch.from_numpy(np.load("./elmo_actions.npy")).to(args.device) # (117, 1024)
        # self.Elmo_object_semantics = torch.from_numpy(np.load("./elmo_object_semantics_81.npy")).to(args.device) # (81, 1024)
        
        self.score_factor_action = 10
        # self.score_factor_object = 10
    ...
    def forward(self, samples: NestedTensor):
        ...
        # text_features_obj = self.Elmo_object_semantics.float()
        # text_features_obj = text_features_obj / text_features_obj.norm(dim=-1, keepdim=True) # 81x1024
        ...
        text_features = self.Elmo_action_semantics
        text_features = text_features / text_features.norm(dim=-1, keepdim=True) # 117x1024
