...
class HungarianMatcherHOI(nn.Module):
    ...
    def __init__(self, cost_obj_class: float = 1, cost_verb_class: float = 1, cost_bbox: float = 1,
        ...
        super().__init__()
        self.cost_obj_class = cost_obj_class ## 1
        self.cost_verb_class = cost_verb_class ## 1
        self.cost_bbox = cost_bbox ## 2.5
        self.cost_giou = cost_giou ## 1
        self.cost_matching = cost_matching ## 1
        self.use_matching = use_matching ## False 
...
def build_matcher_twostage(args):
    return HungarianMatcherHOI_TwoStage(cost_obj_class=args.set_cost_obj_class, ## 1
                                        cost_verb_class=args.set_cost_verb_class,## 1
                                        cost_bbox=args.set_cost_bbox, cost_giou=args.set_cost_giou, ## 2.5, 1
                                        cost_matching=args.set_cost_matching, use_matching=args.use_matching, ## 1, False
                                        topk=args.topk, thres=args.thres) ## 3, 0.5
