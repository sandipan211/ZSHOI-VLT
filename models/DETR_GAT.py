...
# Using GAT for extracting the text features 
class HOIModel(nn.Module):
    """
    DETR Base + GAT + One stage matching.
    """
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False,
        ...
        self.verb_class_embed = nn.Linear(hidden_dim, 1024) ## FFN head for verb class pred (changed from EoID to adopt to ConsNet head)
        ...
        if self.use_matching:
            self.matching_embed = nn.Linear(hidden_dim, 2) # This is the interactive score head 

        # GAT head below - from ConsNet 
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_sub_coord = self.sub_bbox_embed(hopd_out).sigmoid() # sigmoid used for normalized bbox coordinates 
        outputs_obj_coord = self.obj_bbox_embed(hopd_out).sigmoid()
        outputs_obj_class = self.obj_class_embed(hopd_out)

        # text_features_object = self.semblock()['o_emb']
        ...
        text_features_action = self.semblock()['a_emb'] # 117x1024
        text_features_action = text_features_action / text_features_action.norm(dim=-1, keepdim=True) # 117x1024
        outputs_verb_class = outputs_verb_class @ text_features_action.t() # taking the dot product of the verb logits (from the transformer) with the text features of the actions from the GAT
        outputs_verb_class = outputs_verb_class * self.score_factor # multiplying with 10 to get the corresponding action scores
